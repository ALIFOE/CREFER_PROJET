                    ...
                    @forelse($media as $item)
                        <div class="card bg-white rounded-lg overflow-hidden shadow-md">
                            <div class="relative aspect-w-16 aspect-h-9">                    @if($item->type === 'image')
                                    <img src="{{ Storage::url($item->dimensions['medium']['path'] ?? $item->path) }}" 
                                         alt="{{ $item->title }}" 
                                         class="object-cover w-full h-full cursor-pointer media-item"
                                         data-media-src="{{ Storage::url($item->dimensions['large']['path'] ?? $item->path) }}"
                                         data-thumbnail="{{ Storage::url($item->dimensions['thumb']['path'] ?? $item->path) }}"
