...
class FormationController extends Controller
{
    ...
    public function finaliserInscription(FormationInscription $inscription)
    {
        ...
        return view('formations.finaliser-inscription', compact('inscription'));
    }

    public function soumettreDocuments(Request $request, FormationInscription $inscription)
    {
        // Vérifier que l'utilisateur est le propriétaire de l'inscription
        if (auth()->id() !== $inscription->user_id) {
            abort(403, 'Non autorisé');
        ...
        if ($request->hasFile('documents')) {
            foreach ($request->file('documents') as $file) {
                $documentsUploadés[] = $file->store('documents/formations');
            }
        }

        // Mise à jour de l'inscription avec les nouveaux documents
        $autresDocuments = $inscription->autres_documents_paths ?? [];
        $inscription->update([
            'autres_documents_paths' => array_merge($autresDocuments, $documentsUploadés),
            'statut' => 'documents_soumis'
        ]);

        return redirect()->route('formations.mes-inscriptions')
            ->with('success', 'Les documents ont été soumis avec succès.');
    }
}
