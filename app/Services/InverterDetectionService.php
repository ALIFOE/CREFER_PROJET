...
class InverterDetectionService
{
    ...
    private function checkSMA(): bool
    {
        ...
        $password = config('inverters.connections.sma.password');
        
        try {
            // D'abord, essayons de nous connecter
            $loginResponse = Http::timeout(10)
                ->withoutVerifying() // Ignore la vérification SSL
                ->post("https://{$ipAddress}/dyn/login.json", [
                    'right' => 'usr',
                    'pass' => $password
                ]);

            if (!$loginResponse->successful()) {
                Log::info("Tentative de connexion SMA échouée: authentification refusée");
                return false;
            }

            $sessionId = $loginResponse->json()['sid'] ?? null;
            if (!$sessionId) {
                Log::info("Tentative de connexion SMA échouée: pas de session ID");
                return false;
            }

            // Maintenant, essayons de récupérer les valeurs avec la session
            $response = Http::timeout(10)
                ->withoutVerifying()
                ->withHeaders(['Cookie' => "JSESSIONID={$sessionId}"])
                ->get("https://{$ipAddress}/dyn/getDashValues.json");

            $success = $response->successful();

            // Déconnexion propre
            Http::withoutVerifying()
                ->post("https://{$ipAddress}/dyn/logout.json", [
                    'sid' => $sessionId
                ]);
